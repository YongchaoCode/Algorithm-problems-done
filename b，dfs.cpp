...
//优先队列bfs
安琪儿被莫利皮抓住了！他被莫利皮关进了监狱。监狱被描述为 N * M （N， M <= 200） 矩阵。监狱里有墙、路和警卫。
安琪儿的朋友们想救安琪儿。他们的任务是：接近安琪儿。我们假设“接近安琪儿”是为了到达安琪儿停留的位置。当网格中有守卫时，、
我们必须杀死他（或她？）才能进入网格。我们假设我们向上、向下、向右、向左移动需要 1 个单位的时间，杀死一个守卫也需要 1 个单位的时间。
而且我们足够强大，可以杀死所有的守卫。
你必须计算接近安琪儿的最短时间。（当然，我们只能向上、向下、向左和向右移动到边界内的相邻网格。
    
第一行包含两个整数，分别代表 N 和 M。
然后是 N 行，每行都有 M 个字符。“.”代表道路，“a”代表安琪儿，“r”代表安琪儿的每个朋友。
进程到文件末尾。

对于每个测试用例，您的程序应该输出一个整数，代表所需的最短时间。如果这样的数字不存在，您应该输出一行包含“可怜的天使必须终生呆在监狱里”。
...
struct nod {
    int x, y, time;
     bool operator <(const nod &a) const {//这里为什么要加const？？
...
void bfs() {
    q.push({start_x, start_y, 0});//起点入队
    while (!q.empty()) {
        temp = q.top();
        q.pop();
        if (temp.x==end_x&&temp.y==end_y) {//到达终点???
            cout << temp.time << endl;
            return ;
        }
